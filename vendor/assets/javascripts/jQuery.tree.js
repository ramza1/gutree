	...
	return $(this).each(function(){
		...
		tree
			...
			.bind('traverseDown',function(event){
				var target = $(event.target) || tree.find('a[tabindex=0]');
                console.log("traverseDown target",target)
				var targetLi = target.parent();
				if(targetLi.is('[aria-expanded=true]')){
					target.next().find('a').eq(0).trigger("activate")
				}
				else if(targetLi.next().length) {
					targetLi.next().find('a').eq(0).trigger("activate")
				}	
				else {				
					targetLi.parents('li').next().find('a').eq(0).trigger("activate")
				}
			})
			...
			.bind('traverseUp',function(event){
				...
				if(targetLi.prev().length){ 
					if( targetLi.prev().is('[aria-expanded=true]') ){
						targetLi.prev().find('li:visible:last a').eq(0).trigger("activate");
					}
					else{
						targetLi.prev().find('a').eq(0).trigger("activate");
					}
				}
				else { 				
					targetLi.parents('li:eq(0)').find('a').eq(0).trigger("activate");
				}
			})
            .bind('activate',function(event){
                var target = $(event.target)
                tree.find('[tabindex=0]').attr('tabindex','-1').removeClass('tree-item-active');
                target.attr('tabindex','0').addClass('tree-item-active');
                target.focus();
            })
					...
					if(event.keyCode == 37 || event.keyCode == 38 || event.keyCode == 39 || event.keyCode == 40){
						...
						if(event.keyCode == 37){ 
							...
							//try traversing to parent
								...
								target.parents('li:eq(1)').find('a').eq(0).trigger("activate");
							}	
						}						
						...
						if(event.keyCode == 39){ 
							...
							//try traversing to child
								...
								target.parents('li:eq(0)').find('li a').eq(0).trigger("activate");
							}
						}
						//if key is up arrow
						if(event.keyCode == 38){ 
        ...
        if( !$(this).parents('.tree').length ){
            ...
            tree
                ...
                .bind('expand',function(event){
                    var target = $(event.target) || tree.find('a[tabindex=0]');
                    target.removeClass('tree-parent-collapsed');
                    target.next().hide().removeClass('tree-group-collapsed').show()
